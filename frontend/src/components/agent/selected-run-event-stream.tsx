...
export default function LandingSelectedRunEventStream({ runs }: Props) {
    ...
    async function load() {
      try {
        const [latestEvents, stepsResponse] = await Promise.all([
          agentApi.getRunEvents(selectedRun!.id),
          agentApi.getAttemptSteps(selectedRun!.id, (selectedRun as Record<string, unknown>).current_attempt_id as string ?? "", 1, 50),
        ]);
        if (!cancelled) {
  ...
  const items = useMemo(() => {
    ...
    return [...events]
      ...
      .map((event) => ({
        ...
        description: describeEvent(event, selectedRun, stepMap) ?? "该事件没有可解释的自然语言描述。",
    ...
    <div className="landing-event-toggle-block">
      ...
      {open ? (
        <section className="landing-event-panel" aria-label="Agent Loop 事件流">
          ...
          <div className="landing-event-run-chooser">
            {runs.map((run) => (
              <button
                key={run.id}
                type="button"
                className={`landing-event-run-pill ${selectedRun?.id === run.id ? "landing-event-run-pill-active" : ""}`}
